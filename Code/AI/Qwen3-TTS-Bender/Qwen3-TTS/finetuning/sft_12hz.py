...
def train():
    ...
    qwen3tts.model.talker.model.config.use_cache = False
    for param in qwen3tts.model.speaker_encoder.parameters():
        param.requires_grad = False

    if args.train_scope == "low_vram":
        for param in qwen3tts.model.parameters():
            param.requires_grad = False

        for module in (
            qwen3tts.model.talker.text_projection,
            qwen3tts.model.talker.model.codec_embedding,
            qwen3tts.model.talker.codec_head,
        ):
    ...
    accelerator.print(
        f"Training {sum(p.numel() for p in trainable_params):,} parameters with {args.optimizer} ({args.train_scope})"
    )
