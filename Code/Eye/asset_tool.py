...
def animation_frame_index(animation: dict[str, Any], elapsed_ms: int) -> int:
    """Resolve a frame, including an optional intro followed by a range loop."""
    count = len(animation["frames"])
    if count <= 1:
        return 0
    step = elapsed_ms // animation["frame_ms"]
    loop_range = animation.get("loop_range")
    if loop_range is not None and step >= loop_range["start"]:
        start, end = loop_range["start"], loop_range["end"]
        loop_count = end - start + 1
        loop_step = step - start
        if loop_range["mode"] == "ping_pong" and loop_count > 1:
            final = loop_count - 1
            phase = loop_step % (final * 2)
            local = phase if phase <= final else final * 2 - phase
        else:
            local = loop_step % loop_count
        return start + local
    if animation["playback"] == "ping_pong":
        final = count - 1
        phase = step % (final * 2)
        return phase if phase <= final else final * 2 - phase
    return step % count
...
def validate_assets(
    ...
    for state_id, animation in enumerate(animations):
        ...
        if loop_range is not None:
            if (not isinstance(loop_range, dict) or
                    set(loop_range) != {"start", "end", "mode"} or
                    not isinstance(loop_range["start"], int) or
                    not isinstance(loop_range["end"], int) or
                    not 0 <= loop_range["start"] <= loop_range["end"] < len(frames) or
                    loop_range["mode"] not in PLAYBACK_IDS):
                raise ValueError(
                    f"{animation['name']}: invalid loop_range"
                )
        for frame in frames:
...
def compile_assets(
    ...
    for animation in data["animations"]:
        ...
        animation_records.append((
            ...
            loop_range["start"] if loop_range else 0,
            loop_range["end"] + 1 if loop_range else 0,
            PLAYBACK_IDS[loop_range["mode"]] if loop_range else 0,
        ))
    ...
    animations = "\n".join(
        ...
        f"{loop_start}u, {loop_end}u, {loop_playback}u}},"
        for (first, count, frame_ms, playback, loop_start, loop_end,
             loop_playback) in animation_records
    )
    header = f"""// Generated by asset_tool.py from the selected eye asset pack.
...
#define EYE_ASSETS_HAS_LOOP_RANGES 1
...
def insert_animation_frame(
    ...
    loop_range = animation.get("loop_range")
    if loop_range is not None:
        if inserted <= loop_range["start"]:
            loop_range["start"] += 1
            loop_range["end"] += 1
        elif inserted <= loop_range["end"] + 1:
            loop_range["end"] += 1
    write_asset_pack(data, assets_path, header_path)
    return inserted
...
def remove_animation_frame(
    ...
    loop_range = animation.get("loop_range")
    if loop_range is not None:
        if frame_index < loop_range["start"]:
            loop_range["start"] -= 1
            loop_range["end"] -= 1
        elif frame_index <= loop_range["end"]:
            if loop_range["start"] == loop_range["end"]:
                animation.pop("loop_range")
            else:
                loop_range["end"] -= 1
...
def set_animation_timeline(
    ...
    playback: str,
    *,
    loop_range: dict[str, Any] | None = None,
    assets_path: pathlib.Path = DEFAULT_ASSETS,
    header_path: pathlib.Path | None = None,
) -> None:
    ...
    if loop_range is not None:
        if (not isinstance(loop_range, dict) or
                ...
                not 0 <= loop_range["start"] <= loop_range["end"] < len(frames) or
                loop_range["mode"] not in PLAYBACK_IDS):
            raise ValueError(f"invalid loop range for {state}")
    data = load_assets(assets_path)
    timeline = []
    ...
    if loop_range is None:
        animation.pop("loop_range", None)
    else:
        animation["loop_range"] = dict(loop_range)
    prune_unreferenced_sprites(data)
    write_asset_pack(data, assets_path, header_path)
...
def parse_args() -> argparse.Namespace:
    ...
    compile_command.add_argument(
        "--assets", type=pathlib.Path, default=DEFAULT_ASSETS,
    )
    compile_command.add_argument(
        "--output", type=pathlib.Path, default=DEFAULT_HEADER,
    )
...
def main() -> int:
    ...
    try:
        if args.command == "validate":
            ...
            compile_assets(args.assets, args.output)
